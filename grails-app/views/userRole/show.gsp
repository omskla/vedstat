			...
			<ol class="property-list userRole">
				...
				<li class="fieldcontain">
					<span id="users-label" class="property-label"><g:message code="userRole.users.label" default="Users" /></span>
						...
						<span class="property-value" aria-labelledby="users-label"><g:link controller="user" action="show" id="${userRoleInstance?.users?.id}">${userRoleInstance?.users?.encodeAsHTML()}</g:link></span>
