...
<script>
    ...
    function showOk(idAfter) {
        ...
        td.innerHTML ='<a href="${createLink(controller: "statistic",action: "downloadOt4ByArea") }">скачать</a>'
    }
    ...
    function updateTd(data,idTd) {
        $.each(data, function (key, value) {
            ...
            if(key == 'action') {
                ...
                if(value=='aivOt')
                {
                    td.innerHTML = '<a href="${createLink(controller: "statistic",action: "downloadByTemplateName", params: [templateName: 'aivOt.xls']) }">скачать</a>' ;
                }
                if(value=='aivOb')
                    td.innerHTML = '<a href="${createLink(controller: "statistic",action: "downloadByTemplateName", params: [templateName: 'aivOb.xls']) }">скачать</a>' ;
                if(value=='infMonitor')
...
<div id="statistic" class="stat">
    <h1 style="text-align: center"><b>Статистические показатели по району и ОКВЭД</b></h1>
    <table>         <tr>
